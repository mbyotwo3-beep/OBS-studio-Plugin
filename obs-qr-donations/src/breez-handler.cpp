...
BreezHandler::BreezHandler(QObject *parent) : QObject(parent) {
  qInfo() << "BreezHandler stub initialized. Use BreezService for SDK "
             "interactions.";
}

BreezHandler::~BreezHandler() = default;

bool BreezHandler::initialize(const QString &apiKey,
...
QString BreezHandler::createInvoice(qint64 amountSats,
...
qint64 BreezHandler::balance() const {
  ...
  try {
    ...
    return balance.on_chain_balance_sat +
           (balance.off_chain_balance_msat / 1000);
  } catch (const std::exception &e) {
...
void BreezHandler::setupPaymentListener() {
  ...
  m_sdk->set_payment_listener([this](const PaymentReceivedEvent &event) {
    ...
    QMetaObject::invokeMethod(
        this, [this, event]() { onPaymentReceived(event); },
        Qt::QueuedConnection);
  });
}
