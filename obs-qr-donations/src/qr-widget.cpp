...
QRDonationsWidget::QRDonationsWidget(QWidget *parent)
  ...
  d->lightningLoadingLabel->setStyleSheet(
  ...
  connect(d->copyLightningBtn, &QPushButton::clicked, this,
          &QRDonationsWidget::onCopyLightningClicked);
  ...
  lightningLayout->addWidget(
      new QLabel("<b>Lightning Invoice:</b>", lightningTab));
  ...
  d->bitcoinAddressLabel->setStyleSheet(
      "background-color: #f0f0f0; padding: 8px; border-radius: 4px;");

  d->copyBitcoinBtn = new QPushButton("Copy Address", bitcoinTab);
  connect(d->copyBitcoinBtn, &QPushButton::clicked, this,
          &QRDonationsWidget::onCopyBitcoinClicked);
  ...
  d->liquidAddressLabel->setStyleSheet(
      "background-color: #f0f0f0; padding: 8px; border-radius: 4px;");

  d->copyLiquidBtn = new QPushButton("Copy Liquid Address", liquidTab);
  connect(d->copyLiquidBtn, &QPushButton::clicked, this,
          &QRDonationsWidget::onCopyLiquidClicked);
  ...
  connect(d->tabWidget, &QTabWidget::currentChanged, this,
          &QRDonationsWidget::onTabChanged);
  ...
  d->amountHintLabel->setStyleSheet(
      "color: #2196F3; font-style: italic; font-size: 12px;");
  ...
  d->currentMethodLabel->setStyleSheet(
      "font-weight: bold; font-size: 14px; color: #2196F3; padding: 4px;");
  // Simulation banner (visible when the stub is compiled with simulation
  // enabled)
  d->simulationLabel = new QLabel(this);
  d->simulationLabel->setAlignment(Qt::AlignCenter);
  d->simulationLabel->setStyleSheet(
      "background-color: #FFF3CD; color: #856404; padding: 6px; border: 1px "
      "solid #FFE8A1; border-radius: 4px; font-weight: bold;");
#ifdef BREEZ_STUB_SIMULATE
  d->simulationLabel->setText(
      "Demo Mode: Payments are SIMULATED — no real funds will be transferred");
  ...
  connect(d->rotationTimer, &QTimer::timeout, this, [this]() {
    ...
    do {
      ...
      if (idx == 0 && !d->lightningInvoice.empty()) {
        d->tabWidget->setCurrentIndex(0);
        d->currentMethodLabel->setText("Lightning");
        break;
      }
      if (idx == 1 && !d->bitcoinAddress.empty()) {
        d->tabWidget->setCurrentIndex(1);
        d->currentMethodLabel->setText("Bitcoin");
        break;
      }
      if (idx == 2 && !d->liquidAddress.empty()) {
        d->tabWidget->setCurrentIndex(2);
        d->currentMethodLabel->setText("Liquid");
        break;
      }
      attempts++;
  ...
  connect(manageWalletBtn, &QPushButton::clicked, this,
          &QRDonationsWidget::onManageWalletClicked);
  ...
  d->flashOverlay->setStyleSheet("QLabel { "
...
void QRDonationsWidget::onCopyLiquidClicked() {
  ...
  if (clipboard && !d->liquidAddress.empty()) {
    ...
    QMessageBox::information(this, "Copied",
                             "Liquid address copied to clipboard");
  } else {
...
void QRDonationsWidget::setAddress(const std::string &asset,
...
void QRDonationsWidget::setLiquidAddress(const std::string &address) {
  ...
  if (d->liquidAddressLabel)
    d->liquidAddressLabel->setText(QString::fromStdString(address));
  updateQRCode();
}

void QRDonationsWidget::setDisplayOptions(bool showBalance,
...
qint64 QRDonationsWidget::getAmountSats() const { return d->amountSats; }

QImage QRDonationsWidget::getQRCodeImage() const { return d->qrCode; }
  ...
  (void)QtConcurrent::run([this]() {
    try {
      if (BreezService::instance().isReady()) {
        QString description = QString("Donation for %1 stream")
                                   .arg(QString::fromStdString(d->currentAsset));

        // Generate invoice (this might take some time)
        QString invoice =
            BreezService::instance().createInvoice(d->amountSats, description,
                                                   86400 // 24h expiry
            );
      ...
      QString errorMsg =
          QString("Failed to generate invoice: %1").arg(e.what());
      qWarning() << errorMsg;
...
void QRDonationsWidget::updateQRCode() {
  ...
  if (!d->lightningInvoice.empty()) {
    QImage lightningQR = QRGenerator::generateQRCode(
        d->lightningInvoice, d->lightningQRLabel->width() - 20,
  ...
  if (!d->bitcoinAddress.empty()) {
    ...
    QImage bitcoinQR = QRGenerator::generateQRCode(
        qrText.toStdString(), d->bitcoinQRLabel->width() - 20,
  ...
  if (!d->liquidAddress.empty()) {
    ...
    QImage liquidQR = QRGenerator::generateQRCode(
        qrText.toStdString(), d->liquidQRLabel->width() - 20,
...
void QRDonationsWidget::updateLayout() {
  ...
  if (d->copyLightningBtn) {
    d->copyLightningBtn->setEnabled(!d->isLoading &&
                                    !d->lightningInvoice.empty());
  }
  ...
  if (d->showBalance) {
    ...
    d->balanceLabel->setText("Balance: 0.0 " +
...
void QRDonationsWidget::setLightningStatus(const QString &status, bool ok) {
  if (d->lightningStatusLabel) {
    ...
    if (ok) {
      d->lightningStatusLabel->setStyleSheet("color: #4CAF50; font-size: 12px;");
    } else {
      d->lightningStatusLabel->setStyleSheet("color: #f44336; font-size: 12px;");
    }
...
void QRDonationsWidget::showBackupReminder(const QString &seedPath) {
  // Show backup reminder dialog
  BackupReminderDialog dialog(seedPath, this);
  int result = dialog.exec();
  
  // If user completed backup or chose "don't show again", create flag file
  QString workingDir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/breez";
  QString flagPath = workingDir + "/backup_reminder_shown";
  
  if (dialog.backupCompleted() || dialog.dontShowAgain()) {
...
void QRDonationsWidget::onPaymentReceived(qint64 amountSats,
                                          const QString &paymentHash,
                                          const QString &memo) {
  // Update UI to show the payment with flash effect instead of blocking message
  // box
  ...
  if (d->flashOverlay) {
    ...
    d->flashOverlay->setGeometry((width() - overlayWidth) / 2,
                                 (height() - overlayHeight) / 2, overlayWidth,
                                 overlayHeight);
  ...
  // Flash background
  setStyleSheet(
      "QRDonationsWidget { background-color: rgba(76, 175, 80, 50); }");
...
void QRDonationsWidget::onStreamStatusChanged(bool streaming) {
  if (streaming) {
    ...
    if (!d->lightningInvoice.empty()) {
      message +=
          "Lightning Invoice: " + QString::fromStdString(d->lightningInvoice) +
          "\n\n";
    }

    if (!d->bitcoinAddress.empty()) {
      message +=
          "Bitcoin Address: " + QString::fromStdString(d->bitcoinAddress) +
          "\n\n";
    }
    ...
    if (clipboard) {
      ...
      if (!d->lightningInvoice.empty()) {
        clipboardText +=
            "Lightning: " + QString::fromStdString(d->lightningInvoice) + "\n";
      }
      if (!d->bitcoinAddress.empty()) {
        clipboardText +=
            "Bitcoin: " + QString::fromStdString(d->bitcoinAddress) + "\n";
      }
...
void QRDonationsWidget::onCopyLightningClicked() {
  QClipboard *clipboard = QGuiApplication::clipboard();
  if (clipboard && !d->lightningInvoice.empty()) {
    clipboard->setText(QString::fromStdString(d->lightningInvoice));
    QMessageBox::information(this, "Copied",
                             "Lightning invoice copied to clipboard");
  } else {
    QMessageBox::warning(this, "Copy", "No Lightning invoice available to copy");
  }
}

void QRDonationsWidget::onTabChanged(int index) {
  Q_UNUSED(index);
  // Optional: update UI based on which tab is selected
  updateQRCode();
}

void QRDonationsWidget::onGenerateClicked() {
  generateInvoices();
}
