    ...
    , d(new Private)
{
    ...
    d->lightningLoadingLabel->setStyleSheet("background-color: rgba(255, 255, 255, 200);");
    
    QMovie *loadingMovie = new QMovie(":/icons/loading.gif");
    d->lightningLoadingLabel->setMovie(loadingMovie);
    loadingMovie->start();
    
    // Stack the loading indicator on top of the QR code
    d->lightningLoadingLabel->setFixedSize(d->lightningQRLabel->size());
    ...
    d->copyLightningBtn->setIcon(QIcon(":/icons/copy.png"));
    ...
    // Add tabs
    d->tabWidget->addTab(lightningTab, QIcon(":/icons/lightning.png"), "Lightning");
    d->tabWidget->addTab(bitcoinTab, QIcon(":/icons/bitcoin.png"), "Bitcoin");
