...
public:
  ...
  void showBackupReminder(const QString &seedPath);

public slots:
  // Handle payment received
  void onPaymentReceived(qint64 amountSats, const QString &paymentHash,
                         const QString &memo);
...
private:
  ...
  struct Private {
    ...
    QLabel *currentMethodLabel =
        nullptr; // shows which method (Liquid/Lightning/Bitcoin) is currently
                 // displayed
    QLabel *simulationLabel =
        nullptr; // shows when running in simulated stub mode
