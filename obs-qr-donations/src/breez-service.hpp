...
class BreezService : public QObject, public breez_sdk::EventListener {
  Q_OBJECT
...
public:
  ...
  void on_event(const breez_sdk::SdkEvent& e) override;
  ...
  QVariantMap fullNodeInfo() const;
  ...
  // Send on-chain (bitcoin or liquid) to an address. Returns true if operation
  // started
  bool sendOnChain(const QString &address, qint64 amountSats,
                   const QString &network = "bitcoin");
...
signals:
  ...
  void backupReminderNeeded(const QString &seedPath);
