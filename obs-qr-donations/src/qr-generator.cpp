...
static QRcode *generateQRCodeData(const std::string &text,
                                  QRGenerator::ErrorCorrectionLevel level) {
  // Convert error correction level
  QRecLevel ecLevel;
  switch (level) {
  case QRGenerator::ErrorCorrectionLevel::Low:
    ecLevel = QR_ECLEVEL_L;
    break;
  case QRGenerator::ErrorCorrectionLevel::Medium:
    ecLevel = QR_ECLEVEL_M;
    break;
  case QRGenerator::ErrorCorrectionLevel::Quartile:
    ecLevel = QR_ECLEVEL_Q;
    break;
  case QRGenerator::ErrorCorrectionLevel::High:
  ...
  // Generate QR code
  QRcode *qr = QRcode_encodeString(text.c_str(), // Text to encode
                                   0,            // Version (0 = auto)
...
QImage QRGenerator::generateQRCode(const std::string &text, int width,
                                   int height) {
  return generateQRCode(text, width, height, ErrorCorrectionLevel::Medium);
}

QImage QRGenerator::generateQRCode(const std::string &text, int width,
                                   int height, ErrorCorrectionLevel level) {
  if (text.empty()) {
    return QImage();
  }
  ...
  if (!qr) {
    qWarning() << "Failed to generate QR code for text:"
